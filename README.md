...
| **Statement spec** | **Lean FRO [comparator](https://github.com/leanprover/comparator)** ([`proofs/comparator/`](proofs/comparator/)) | *Did we prove what we claim?* [`Challenge.lean`](proofs/comparator/Challenge.lean) is a self-contained, human-auditable spec: every definition the statements depend on plus all 34 audited theorems restated with `sorry`. On every push, the comparator verifies the real proofs prove **exactly** those statements, use only the standard axiom base (`propext` / `Quot.sound` / `Classical.choice`), and replay in the Lean kernel — and CI separately asserts the solution file is byte-for-byte the four proof files. A reviewer only needs to read `Challenge.lean` |
